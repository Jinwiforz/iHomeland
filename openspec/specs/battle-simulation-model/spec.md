...
物理访问 MUST 只通过项目 `PhysicsWorld` value contract 提供的 ground probe、capsule move、shape/ray/overlap 与 projectile sweep；结果 MUST 规范排序且不得暴露 Jolt 类型或 callback 顺序。纯模型 fixtures MUST 能以记录化 physics result 替代真实物理库。
