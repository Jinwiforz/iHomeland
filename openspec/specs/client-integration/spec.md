...
iHomeland 第一阶段客户端接入文档和工程规划必须以 Unity 为目标，不得继续把 Godot 作为并列待选客户端方向。
...
Unity 客户端必须以 `shared/proto/` 中的 `.proto` 文件作为协议源，生成 C# 协议代码；生成代码不得手工修改。
...
Unity 客户端连接实时网关时必须使用 WebSocket `/ws`，并以二进制帧收发 Protobuf envelope。
...
Unity 客户端必须正确设置和处理 `protocol_version`、`message_id`、`request_id`、`sequence` 和 payload。
...
Unity 客户端必须周期性发送心跳，并能够展示或记录服务端结构化错误。
...
Unity 客户端执行房间大厅操作后，必须以服务端返回的 `RoomSnapshot` 作为 UI 状态来源。
...
Unity 客户端接入文档必须提供本地服务端启动、版本检查、WebSocket 连接、心跳和房间大厅请求的最小验收路径。
