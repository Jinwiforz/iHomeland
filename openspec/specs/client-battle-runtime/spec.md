...
Client codec MUST 在 generated Protobuf mapping 前验证 message `3002/3003` field 7 显式存在，并只在完整 partition set 通过 current generation、sequence、baseline 和 entity validation 后应用 `last_processed_input_tick`。Acknowledgement MUST 单调；合法上限 MUST 为current generation已实际发送或显式前向跳过的最大InputTick，与由同一committed ServerTick按冻结6-Tick gap-expiry严格不等式计算出的最大已终结InputTick之大者，显式0表示尚未终结InputTick 1。超过该数学上限、ack回退或ServerTick回退必须fail closed；不得使用snapshot计数、等待时长或frontier稳定猜测。只有`GameplayPrediction`已因长帧或有界历史过载显式进入lost-continuity、清空旧预测且关闭input gate时，下一份current authoritative snapshot MAY把服务器已经终结的更高input frontier收为continuity anchor；该例外不得适用于predecessor generation或仍保留未确认输入的状态。`GameplayPrediction` MUST 从权威 local actor state恢复确认基点、删除已确认历史并按原顺序重演未确认输入；位置误差超过80 mm或角度误差超过2000 millidegree时 MUST 更新事实状态，Scene只可通过有界表现轨迹收敛。
...
客户端 MUST 只在 authenticated ServerAccept 后按 wire v1 actor mapping 把 local entity identity 解析为 one-based actor slot（`entity_id = actor_slot + 1`），并以 current binding/generation 验证该 entity 存在；payload、Scene 或 View不得选择 local actor。`GameplayPrediction` MUST 从latest authority到future input horizon逐个50 ms SimulationTick重演：有frame组按last Move/Aim与OR Jump只积分一次，无frame的中间Tick按与C++相同的4-Tick闭区间continuous hold复用last Move；send pacing等待只可保留一个25 ms credit，不能在10 Hz publication放行帧catch-up burst。Prediction owner MUST 从current visible pose到完整horizon发布唯一、frame-rate-independent且不越过endpoint的`PresentationTransform`；continuity loss必须冻结current可信pose，death/full baseline必须锚定authority。Local Actor View MUST 在同帧精确提交该transform，不得在Unity Scene建立第二只segment计时器、读取raw Move或按velocity外推。Camera MUST 跟随同一Transform。远端玩家、怪物、Boss 与投射物 MUST 由 `GameplayInterpolation` 使用 100 ms render delay 的有界 samples插值，并最多外推 150 ms，超过窗口后冻结/降级而不是继续猜测。两条路径都 MUST 保留服务器 snapshot 为权威事实。

#### Scenario: 本地 prediction cadence 低于 render cadence

- **WHEN**local input以25 ms采样、movement以50 ms积分、authority每2 Tick发布一次，两个publication之间的中间Tick没有新frame但仍处于continuous hold窗口
- **THEN**GameplayPrediction逐Tick重演held Move并以独立于send credit的prediction-owned timeline发布连续`PresentationTransform`；放行帧不burst，Actor View同帧精确提交、Camera跟随同一输出且下一等价authority不产生约一步movement rebase
