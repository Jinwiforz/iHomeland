...
客户端 HTTP adapter MUST 以强类型方法实现 `getVersion`、`getBootstrapConfig`、`registerAccount`、`loginAccount`、`refreshSession`、`logoutSession`、`issueConnectionTicket` 与 `getWorldBootstrap` 的既有 method、path、认证、body、成功 status 和 JSON schema。请求 MUST 只包含 OpenAPI 声明字段；Bearer header MUST 只来自 Session owner 的当前 access token。`acceptVisitInvite` 与 `issueWorldAdmission` 不属于本 change，HTTP 核心不得以通用任意 path/body 接口绕过该边界。
