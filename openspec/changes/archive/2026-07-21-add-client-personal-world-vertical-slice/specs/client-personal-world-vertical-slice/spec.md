...
### Requirement: 产品入口必须从本地登录页面显式启动

客户端 MUST 在 App Scope 全部初始化成功后通过唯一 Experience 打开 `Login` route，但 MUST NOT 因页面显示自动访问网络。Register/login intent MUST 先完成冻结 version/config bootstrap，再调用唯一 Session owner；认证成功后 MUST 启动唯一 control run，并在该 run 首次进入 `Connected` 后显式请求进入自己的 PersonalWorld。若 control 在 `Connected` 前稳定失败，客户端 MUST 保留已提交 Session、停止进入世界并呈现可重试的低敏连接失败，不能在无法接收定向 invite push 时伪装产品流程已就绪。Password MUST 只存在于当前输入与调用参数，页面隐藏、失败、停止或提交完成后 MUST 清除，不得进入 View State、日志、Scene、Prefab 或异常文本。

#### Scenario: 新进程启动

- **WHEN** Windows Player 在没有可恢复 session 的当前基线下完成 App Scope 初始化
- **THEN** 只显示 Login 页面且不发出 version、config、register、login、ticket、world 或 channel 请求

#### Scenario: 登录并进入自己的世界

- **WHEN** 玩家提交合法登录信息且 bootstrap、login、control start 和 own-world admission 依次成功
- **THEN** Session owner 保存唯一认证事实，Experience 离开 Login 并进入 own-world loading/scene 流程，View 与 Scene 不取得 token、ticket 或 admission
...
Experience MUST 将 protocol incompatibility、unauthenticated/session invalidation、permission、validation、revision conflict、not found、rate limit、dependency unavailable、transport/timeout 和内部失败映射为封闭低敏 UI failure。Session invalidation MUST 取消当前 presentation/scene intent、关闭产品 routes 和 Scene Scope，并回到 Login；session 仍有效的 control/gameplay 中断 MUST 使用 `ConnectionLost` 或 Shell retry 状态，不得伪造自动恢复成功。

#### Scenario: Session 在 Visiting 时失效

- **WHEN** forced logout 或 session invalidation 使唯一 Session owner 清除当前 lineage
- **THEN** Experience 取消 target/scene/page intent、关闭旧 HUD/overlay、卸载内容 Scene 并回到 Login，任何旧 generation completion 都不能重新认证或打开 world route
...
- **THEN** channel owner 事件驱动使当前 world mutation 与 Scene/HUD 立即失效并呈现稳定连接失败及显式 retry/logout；显式 target 切换、safe-return、logout 或 shutdown 的关闭不得误报为 server failure

#### Scenario: 服务端恢复后显式重新连接

- **WHEN** Session 仍有效、客户端已显示 `ConnectionLost`，服务端恢复后玩家提交一次 reconnect
- **THEN** 客户端通过 control readiness 事件等待 Connected，再依次重建 own-world target、Scene 与 HUD，全部提交后才关闭 modal；不得轮询 channel state、使用固定延时或只恢复 WSS 而保留失效 gameplay/Scene，route 自身关闭也不能取消恢复事务

#### Scenario: 重连时 access 已到期但 refresh 仍有效

- **WHEN** 玩家提交显式 reconnect，唯一 Session owner 的 access 已越过服务端签发的绝对 expiry，refresh lineage 仍有效
- **THEN** 首个授权 HTTP operation 必须先共享一次 single-flight refresh，再以新 session generation 和新 access 继续 control ticket、own-world admission 与 Scene/HUD 恢复；不得先发送已知过期 access、显示内部错误 key、要求玩家重复登录或循环重试

#### Scenario: 到期刷新无法确认提交结论

- **WHEN** access 到期触发的 refresh 返回 timeout、transport 或 commit-unknown
- **THEN** Session owner 必须按既有 refresh 契约使旧 token lineage fail closed，停止原始授权 operation并显示低敏可恢复状态；不得继续使用旧 access 或猜测 refresh 未提交
...
- **THEN** 客户端必须接纳新的 runtime assignment 并完成 admission；同 generation 的 instance/endpoint 漂移、generation 回退或 lease deadline 回退仍必须 fail closed
