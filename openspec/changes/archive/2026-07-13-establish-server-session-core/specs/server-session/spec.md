...
- **THEN** 服务端从受信 endpoint provider 选择 TLS/TCP endpoint，只授予 gameplay scope，后续业务 command 仍需有效 admission 与 application authorization
...
- **THEN** 它只查询 AuthContext scope；执行具体业务 mutation 时还必须校验 admission、actor role 和领域 policy
