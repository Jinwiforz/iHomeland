...
唯一 B0.6 入口 MUST 提供 `validate`、定向 `diagnose`、完整 `verify`、长时 `soak` 与 `finalize` 语义；只有 `finalize` 可以消费同一 identity 下连续两次完整 verify、mandatory soak、安全矩阵、既有 B0.3/B0.4/B0.5、server v1/client v1 regression 和 OpenSpec strict evidence，生成 `battle-network-qualified-windows-x64-controlled` 报告。报告 MUST 绑定全部 source/binary/config/environment/fault/workload digest，逐场景记录 measured value、unit、budget、disposition、run evidence digest、cleanup 和 scope；missing、failed、skipped、stale、unsupported、unclassified 或 cleanup failure 任一存在时 MUST not-qualified。Tracked corpus 与报告不得包含 raw ticket、proof/traffic key、cookie secret、完整 credential、玩家资料、payload dump 或本机绝对路径。

#### Scenario: 连续运行结论不一致

- **WHEN** 两次完整 verify 的 identity 相同但任一 mandatory scenario disposition 不同、测量超出可重复容差或 cleanup 不一致
- **THEN** finalize 拒绝生成 qualified report，并要求保留两个低敏 run digest 供定位

#### Scenario: 全部门禁通过

- **WHEN** 两次 verify、30 分钟 soak、安全负例、回归、strict validation 与 cleanup 在同一冻结 identity 下全部通过
- **THEN** 报告可声明 `battle-network-qualified-windows-x64-controlled` 并只解锁 B0.7，不声明 Linux、公网运营商或 Unity runtime 已 qualified
