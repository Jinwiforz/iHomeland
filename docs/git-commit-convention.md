...
本文档是 iHomeland Git 提交消息与提交粒度的唯一权威文档。所有人工提交、AI 辅助提交、squash 结果和自动化提交都必须遵守本规范。
...
- 新 scope 必须对应长期权威边界，不能为单次提交随意造词。
...
- 实现 OpenSpec 变更的提交应写 `OpenSpec: <change-name>`。
...
4. 架构、协议、存储或流程破坏性变更必须关联 OpenSpec 变更。
...
4. OpenSpec tasks、主 specs 和权威文档已按实际进度更新。
