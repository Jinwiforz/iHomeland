...
Input System 只提交量化 Move/Aim/Jump/Primary/Secondary/Interact intent。运行时 Player map 默认精确配对 Keyboard 与 Mouse；只有具体 Gamepad 的显式按键可切换到该设备，未配对虚拟手柄、Joystick、XR 与 Touch 的轴噪声不能参与聚合。共享 `<Pointer>/delta` 绑定必须通过设备配对保留鼠标语义，不能用 Touch binding-group override 连带清空。Local movement/jump prediction、acknowledgement pruning 与 reconciliation 位于纯 C# Application；25 ms InputTick只用于采样和发送，同一50 ms SimulationTick内以last move/aim、OR jump从共同起点单次积分，连续ACK缺口保留的frame只有映射到latest authority ServerTick之后才允许重演，current平地落点使用与server一致的百万分比crossing与toward-zero舍入。正常authority snapshot以10 Hz每2 Tick发布，因此prediction replay还必须为future frame前的无frame中间Tick复用4-Tick continuous hold内的last Move；send pacing只保留一个InputTick credit且放行不burst。Prediction owner以独立表现时钟从current visible pose连接完整horizon并发布唯一量化`PresentationTransform`；Scene只同帧提交该值，不建立第二个计时器、不额外积分raw Move或按velocity外推，因此不会遗漏一步后再由authority周期性补回。ActorView只从该immutable Transform的量化水平速度派生`Moving`，production Animator在移动期使用保持`VisualRoot`零位移/单位缩放的共享in-place Locomotion，Idle垂直呼吸不得与位移同时播放。snapshot replica与使用authority render delay的remote interpolation仍只读服务器事实。Raw input/snapshot 不回退到 WSS/TLS-TCP，reliable ability/lifecycle/resync 只走登记 KCP lane。
...
## B0.8 production combat 接入

Battle full baseline 现在必须包含 active players、3 个 ordinary monsters、Boss 与 live projectiles，并建立 current archetype、weapon、health/max-health、phase/dead 和 entity generation。客户端仅提交 Move/Aim/Jump/Primary/Secondary/Interact/SwitchWeapon intent；Primary、SwitchWeapon 不携带 hit、target、damage、weapon ID 或 cooldown。Reliable ability/lifecycle event 只驱动表现和 lifecycle convergence。

归档前的定向入口为 `tools/quality/quality.ps1 check-change -Change deliver-personal-world-combat-slice`，其 closed plan 现保存在 `openspec/changes/archive/2026-08-06-deliver-personal-world-combat-slice/validation.json`。其中 `personal-world-combat-targeted` 复用锁定 Unity build owner、真实 Go parent/C++ child、MySQL/Redis 与双 Development Player harness；输出仅进入 ignored `.local/`，不生成最终资格结论。归档后的任何修改必须通过新 OpenSpec change 声明影响面并执行自己的 closed validation，不得复用本次归档结论。Unity 内容制作与 Input Actions/Scene 接线完成前，该 check 应 fail closed，不允许以 generic prefab 或缺失资源视为竖切完成。
