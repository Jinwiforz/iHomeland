...
namespace IHomeland.Client.PersonalWorld.Runtime.Scenes
{
    ...
    public sealed class ClientWorldSceneTransitionHost : MonoBehaviour, IClientWorldSceneTransition
    {
        ...
        /// <summary>
        /// 标识Unity已经接管Player退出或Editor离开Play Mode，实际Scene销毁不再由本Host等待。
        /// </summary>
            ...
            try
            {
                           ...
                           cancellationToken))
                {
...
#if DEVELOPMENT_BUILD || UNITY_EDITOR
                        Debug.LogError(
                            "[IHOMELAND_SCENE_TRANSITION] " +
                            $"outcome=invalid-context reason={exception.Message}");
#endif
                        await RollbackCandidateAsync(candidate, context, candidateLifetime);
...
#endif
                    {
                        ...
                        Debug.LogError(
                            "[IHOMELAND_SCENE_TRANSITION] " +
                            $"outcome=load-failed exception={exception.GetType().Name}");
#endif
                        await RollbackCandidateAsync(candidate, context, candidateLifetime);
                        return SetResult(ClientWorldSceneTransitionCode.LoadFailed, requestGeneration);
                    }
        ...
        /// <summary>
        /// 在Unity退出流程开始时同步关闭新转换资格并取消候选，避免随后等待Unity自己的Scene销毁。
        /// </summary>
        /// <remarks>
        /// AppLifetime仍会调用StopAsync完成Context、SceneLifetime与本地引用清理；本方法只冻结退出语义。
        /// </remarks>
        ...
        /// <summary>永久拒绝新转换，并按正常停止或Unity退出语义释放current内容Scene所有权。</summary>
        ...
        public async Task StopAsync(CancellationToken cancellationToken)
        {
            ...
            lock (_sync)
            {
                ...
                applicationQuitting = _applicationQuitting;
            }
            ...
            if (applicationQuitting)
            {
                try
                {
                    await UnloadCurrentCoreAsync(cancellationToken, skipUnitySceneUnload: true);
                }
                finally
                {
                    CompleteStop(lifetime);
                }
        ...
        /// <summary>释放App Scope取消源并提交Scene Host停止终态。</summary>
        /// <param name="lifetime">本次停止捕获的App Scope转换取消源。</param>
        private void CompleteStop(CancellationTokenSource lifetime)
        {
            lifetime?.Dispose();
            lock (_sync)
            {
        ...
        /// <summary>先失效 SceneLifetime 和 Context，再按调用方所有权决定是否卸载current Unity Scene。</summary>
        /// <param name="cancellationToken">卸载等待取消信号。</param>
        /// <param name="skipUnitySceneUnload">Unity已接管退出时为true，只清理本Host的语义所有权。</param>
        /// <returns>Current scene 已清除时完成。</returns>
        private async Task UnloadCurrentCoreAsync(
            CancellationToken cancellationToken,
            bool skipUnitySceneUnload = false)
        {
            Scene scene;
            PersonalWorldSceneContext context;
            ...
            if (!skipUnitySceneUnload && scene.IsValid() && scene.isLoaded)
            {
            ...
            SceneLifetime lifetime)
        {
            ...
            if (!IsApplicationQuitting() && scene.IsValid() && scene.isLoaded)
            {
                var unload = SceneManager.UnloadSceneAsync(scene);
                if (unload != null)
                {
        ...
        /// <summary>读取Unity是否已经接管退出时的Scene销毁。</summary>
        /// <returns>退出通知已经冻结Host时返回true。</returns>
        private bool IsApplicationQuitting()
        {
            lock (_sync)
            {
                return _applicationQuitting;
            }
        }
