...
namespace IHomeland.Client.PersonalWorldCombat.Application
{
    ...
    internal sealed class ClientBattleRuntimeQualificationSnapshot
    {
        ...
        internal ClientBattleRuntimeQualificationSnapshot(
            ...
            uint bossPhase,
            bool bossDead,
            ulong observedAbilityEventCount,
            ulong observedLocalAbilityEventCount,
            uint lastObservedLocalAbilityID,
            ...
            bool resyncPending)
        {
            ...
            if (entityCount < 0 ||
                ...
                (runtime.BaselineReady &&
                 !ClientBattleContentIdentity.IsKnownWeapon(
                     localEquippedWeaponID)) ||
                (localMaxHealthMilli == 0) !=
                    (localHealthMilli == 0 && !localDead) ||
                localHealthMilli > localMaxHealthMilli ||
                (bossMaxHealthMilli == 0) !=
                    (bossHealthMilli == 0 && bossPhase == 0 && !bossDead) ||
            ...
            LocalEquippedWeaponID = localEquippedWeaponID;
            ...
            ObservedLocalAbilityEventCount =
                observedLocalAbilityEventCount;
        ...
        /// <summary>获取current local player的权威装备武器。</summary>
        internal uint LocalEquippedWeaponID { get; }

        /// <summary>获取current local player的权威生命。</summary>
        internal uint LocalHealthMilli { get; }

        /// <summary>获取current local player不可变生命上限。</summary>
        internal uint LocalMaxHealthMilli { get; }

        /// <summary>获取current local player是否已由权威Death stage终结。</summary>
        internal bool LocalDead { get; }

        /// <summary>获取可见Boss当前权威生命；Boss不存在时为零。</summary>
        internal uint BossHealthMilli { get; }

        /// <summary>获取可见Boss不可变生命上限；Boss不存在时为零。</summary>
        internal uint BossMaxHealthMilli { get; }

        /// <summary>获取可见Boss的权威phase。</summary>
        internal uint BossPhase { get; }

        /// <summary>获取Boss是否已由权威Death stage终结。</summary>
        ...
        IClientBattleInboundSink
    {
            ...
            out ClientBattleRuntimeQualificationSnapshot snapshot)
        {
            ...
            ClientBattleEntityState boss = null;
            for (var index = 0; index < replica.Entities.Count; index++)
            {
                if (replica.Entities[index].ArchetypeID ==
                    ClientBattleContentIdentity.BossArchetype)
                {
                    boss = replica.Entities[index];
                    break;
                }
            }
            ...
            snapshot = new ClientBattleRuntimeQualificationSnapshot(
                ...
                local == null ? 0 : local.EquippedWeaponID,
                local == null ? 0 : local.HealthMilli,
                local == null ? 0 : local.MaxHealthMilli,
                local != null &&
                    (local.StateFlags &
                     ClientBattleEntityState.DeadStateFlag) != 0,
                boss == null ? 0 : boss.HealthMilli,
                boss == null ? 0 : boss.MaxHealthMilli,
                boss == null ? 0 :
                    boss.StateFlags &
                ...
                boss != null &&
                    (boss.StateFlags &
                     ClientBattleEntityState.DeadStateFlag) != 0,
                replica.ObservedAbilityEventCount,
                replica.ObservedLocalAbilityEventCount,
                replica.LastObservedLocalAbilityID,
