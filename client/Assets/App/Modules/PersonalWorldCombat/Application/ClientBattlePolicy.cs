...
namespace IHomeland.Client.PersonalWorldCombat.Application
{
    ...
    internal sealed class ClientBattlePolicy
    {
            ...
            int jumpSpeedMillimetersPerSecond)
        {
            ...
            InputGapExpiryTicks = inputGapExpiryTicks;
        ...
        /// <summary>获取服务端缺失InputTick稳定终结窗口，单位SimulationTick。</summary>
        internal int InputGapExpiryTicks { get; }
        ...
        /// <summary>
        /// 计算authority在current ServerTick可按gap expiry合法终结的最大InputTick。
        /// </summary>
        /// <param name="latestServerTick">本次完整publication的committed ServerTick。</param>
        /// <returns>尚无InputTick满足严格expiry不等式时为零，否则返回连续终结上限。</returns>
        internal ulong LastGapExpiredInputTick(ulong latestServerTick)
        {
            if (latestServerTick <= (ulong)InputGapExpiryTicks + 1UL)
            {
                return 0;
            }
            ...
            var ratio =
                (ulong)(SimulationStepMilliseconds / InputStepMilliseconds);
            var lastExpiredSimulationTick =
                latestServerTick - (ulong)InputGapExpiryTicks - 1UL;
            return checked(lastExpiredSimulationTick * ratio);
        }
    }
}
