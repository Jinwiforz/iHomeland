        ...
        IClientBattleGameplayPort
    {
            ...
            try
            {
                        ...
                        out var partition))
                {
                    ReportTerminalDiagnostic(
                        generation,
                        "raw-snapshot-decode-" +
                        _protocol.LastSnapshotDecodeFailure,
                        null);
                    return false;
                }
        ...
        private bool HandleKcpMessage(long generation, byte[] message)
        {
            ...
            try
            {
                ...
                {
                    if (!_protocol.TryDecodeAbilityEvent(
                            generation,
                            frame,
                            out var ability))
                    {
                    ...
                    if (_inbound.AcceptAbilityEvent(ability))
                    {
                        return true;
                    }
                ...
                {
                    if (!_protocol.TryDecodeEntityLifecycle(
                            generation,
                            frame,
                            out var lifecycle))
                    {
                        ReportTerminalDiagnostic(
                            generation,
                            "kcp-lifecycle-decode",
                            null);
                        return false;
                    }

                    if (_inbound.AcceptEntityLifecycle(lifecycle))
                    {
                        return true;
                    }
                ...
                {
                    if (!_protocol.TryDecodeResyncResponse(
                            generation,
                            frame,
                            out var response))
                    {
                    ...
                    if (_inbound.AcceptResyncResponse(
                            response,
                            _clock.NowMilliseconds))
                    {
                        return true;
