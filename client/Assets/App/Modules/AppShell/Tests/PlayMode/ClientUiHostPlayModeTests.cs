...
namespace IHomeland.Client.AppShell.Tests.PlayMode
{
    /// <summary>
    ...
    public sealed class ClientUiHostPlayModeTests
    {
        ...
        /// <summary>
        /// 保护runtime clone只配对一个设备集合，并保持shared Mouse Aim与模板只读。
        /// </summary>
        /// <returns>等待设备切换、clone初始化与销毁的枚举器。</returns>
        [UnityTest]
        public IEnumerator PlayerDevicePairingPreventsVirtualAxisFallbackAndPreservesMouseAim()
        {
            ...
            try
            {
                ...
                hostRoot.gameObject.SetActive(true);
                Assert.That(
                    hostRoot.InitializeAsync(CancellationToken.None).
                        IsCompletedSuccessfully,
                    Is.True);

                var runtimeField = typeof(ClientUiHostRoot).GetField(
                    "_runtimeInputActions",
                    BindingFlags.Instance | BindingFlags.NonPublic);
                Assert.That(runtimeField, Is.Not.Null);
                var runtime = runtimeField.GetValue(hostRoot) as InputActionAsset;
                Assert.That(runtime, Is.Not.Null);
                var runtimePlayer = runtime.FindActionMap("Player");
                Assert.That(inputAsset.FindActionMap("Player").devices, Is.Null);
                Assert.That(runtimePlayer.devices, Is.Not.Null);
                CollectionAssert.AreEquivalent(
                    new InputDevice[] { keyboard, mouse },
                    runtimePlayer.devices.Value);

                foreach (var binding in inputAsset.FindActionMap("Player").bindings)
                {
                    Assert.That(binding.overridePath, Is.Null);
                }

                InputSystem.QueueStateEvent(
                    gamepad,
                    new GamepadState { leftStick = Vector2.down });
                InputSystem.QueueDeltaStateEvent(
                    joystick.stick,
                    Vector2.down);
                InputSystem.Update();
                Assert.That(
                    runtimePlayer.FindAction("Move").ReadValue<Vector2>(),
                    Is.EqualTo(Vector2.zero),
                    "未配对虚拟轴不能成为Keyboard/Mouse release后的后备值。");
                ...
                Assert.That(
                    runtimePlayer.FindAction("Move").ReadValue<Vector2>(),
                    Is.EqualTo(Vector2.zero),
                    "WASD release必须立即回到neutral，不能回落到未配对Gamepad/Joystick。");

                InputSystem.QueueStateEvent(
                    mouse,
                    new MouseState { delta = new Vector2(8f, -4f) });
                InputSystem.Update();
                Assert.That(
                    runtimePlayer.FindAction("Aim").ReadValue<Vector2>(),
                    Is.EqualTo(new Vector2(8f, -4f)),
                    "Keyboard&Mouse;Touch共享Pointer binding不能因Touch隔离而失去Mouse delta。");
                ...
                InputSystem.Update();
                Assert.That(
                    runtimePlayer.FindAction("Move").ReadValue<Vector2>().x,
                    Is.GreaterThan(0.9f));
                ...
                Assert.That(
                    hostRoot.StopAsync(CancellationToken.None).
                        IsCompletedSuccessfully,
                    Is.True);
                yield return null;
            }
            ...
            {
                foreach (var device in new InputDevice[]
                         {
                             joystick,
                             gamepad,
                             mouse,
        ...
        public IEnumerator GameplayMenuAndUiCancelOnlyPublishForTheirOwningModes()
        {
            ...
            try
            {
                ...
                yield return null;
                Assert.That(
                    hostRoot.CaptureBattleInput().GameplayAvailable,
                    Is.EqualTo(UnityEngine.Application.isFocused),
                    "Gameplay sample 必须同时服从 owner release gate 与 Player focus。批处理 Player 不伪造焦点。");
        ...
        private InputActionAsset CreateInputAsset(bool sharedMenuAndCancelBinding = false)
        {
            ...
            player.AddAction("SwitchWeapon", InputActionType.Button);
            player.AddAction("Menu", InputActionType.Button).AddBinding(
                "<Keyboard>/tab",
