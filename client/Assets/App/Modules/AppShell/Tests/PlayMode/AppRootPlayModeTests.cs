...
namespace IHomeland.Client.AppShell.Tests.PlayMode
{
    ...
    public sealed class AppRootPlayModeTests
    {
        ...
        /// <summary>
        /// 保护Unity接管退出后Scene Host只失效语义所有权，不等待重复的显式Scene unload。
        /// </summary>
        /// <returns>等待测试创建的临时Scene在断言后由测试显式卸载。</returns>
        [UnityTest]
        public IEnumerator ApplicationQuitPreparationSkipsOwnedSceneUnloadWait()
        {
            var hostObject = new GameObject("ApplicationQuitSceneHost");
            var lifetimeOwner = new SceneLifetimeOwner();
            var host = hostObject.AddComponent<ClientWorldSceneTransitionHost>();
            host.Configure(lifetimeOwner);
            Assert.That(
                lifetimeOwner.InitializeAsync(CancellationToken.None).IsCompletedSuccessfully,
                Is.True);
            Assert.That(
                host.InitializeAsync(CancellationToken.None).IsCompletedSuccessfully,
                Is.True);

            var contentScene = SceneManager.CreateScene(
                "ApplicationQuitOwnedScene-" + System.Guid.NewGuid().ToString("N"));
            var sceneLifetime = lifetimeOwner.BeginScene();
            SetPrivateField(host, "_currentScene", contentScene);
            SetPrivateField(host, "_currentLifetime", sceneLifetime);

            host.PrepareForApplicationQuit();
            var stop = host.StopAsync(CancellationToken.None);

            Assert.That(stop.IsCompletedSuccessfully, Is.True, stop.Exception?.ToString());
            Assert.That(contentScene.isLoaded, Is.True);
            Assert.That(lifetimeOwner.IsCurrent(sceneLifetime.Generation), Is.False);
            Assert.That(host.Snapshot.LastResult, Is.EqualTo(ClientWorldSceneTransitionCode.Stopped));

            Object.DestroyImmediate(hostObject);
            yield return SceneManager.UnloadSceneAsync(contentScene);
            Assert.That(
                lifetimeOwner.StopAsync(CancellationToken.None).IsCompletedSuccessfully,
                Is.True);
        }
