...
namespace IHomeland.Client.AppShell.Runtime.Presentation.Hosts
{
    /// <summary>
    ...
    [MovedFrom(true, "IHomeland.Client.Presentation.Hosts", "IHomeland.Client.Runtime", null)]
        ...
        IClientBattleInputSource
    {
        ...
        /// <summary>保存 Player map 中唯一 battle SwitchWeapon action。</summary>
        private InputAction _battleSwitchWeaponAction;

        /// <summary>
        /// 保存Player map当前显式配对的exact Gamepad；null表示Keyboard/Mouse设备集合。
        /// </summary>
        private InputDevice _pairedGameplayGamepad;

        /// <summary>
        /// 保存Input System event callback请求、等待帧末提交的exact Gamepad；null表示Keyboard/Mouse。
        /// </summary>
        private InputDevice _pendingGameplayGamepad;

        /// <summary>表示帧末仍需提交一次Player map设备配对变化。</summary>
        private bool _gameplayDevicePairingPending;

        /// <summary>表示唯一Input System event订阅已登记并必须在clone销毁前撤销。</summary>
        private bool _inputEventSubscribed;
        ...
        internal void ValidateBattleInputConfiguration()
        {
            ...
            ValidateBattleAction(
                ...
                InputActionType.Button,
                "Button");
            ValidateBattleDeviceBindings(player, "Primary");
            ValidateBattleDeviceBindings(player, "SwitchWeapon");
        }
        ...
        internal ClientBattleSceneInputFrame CaptureBattleInput()
        {
            if (!_initialized ||
                ...
                _battleSwitchWeaponAction == null)
            {
                return default;
            }
            ...
            return new ClientBattleSceneInputFrame(
                ...
                sample: new ClientBattleSceneInputSample(
                    ...
                    !suppressTransitionEdges &&
                        _battleSwitchWeaponAction.WasPressedThisFrame()));
        }
        ...
        public Task InitializeAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                _battleInteractAction = _playerActionMap.FindAction(
                    "Interact",
                    throwIfNotFound: false);
                _battleSwitchWeaponAction = _playerActionMap.FindAction(
                    "SwitchWeapon",
                    throwIfNotFound: false);
                PairPlayerMapToGameplayDevice(gamepad: null);
        ...
        public Task StopAsync(CancellationToken cancellationToken)
        {
            ...
            _gameplaySampleWarmupPending = false;
            _pendingGameplayGamepad = null;
            _gameplayDevicePairingPending = false;
        ...
        {
            CommitPendingGameplayDevicePairing();
        ...
        /// <summary>验证关键离散 action 同时具有明确 keyboard/mouse 与 gamepad binding。</summary>
        /// <param name="player">Input asset 中唯一 Player action map。</param>
        /// <param name="actionName">待验证的 production semantic action。</param>
        private static void ValidateBattleDeviceBindings(
            InputActionMap player,
            string actionName)
        {
            var action = player.FindAction(actionName, throwIfNotFound: true);
            var hasKeyboardOrMouse = false;
            ...
            foreach (var binding in action.bindings)
            {
                ...
                hasGamepad |= path.StartsWith("<Gamepad>/", StringComparison.Ordinal);
            }

            if (!hasKeyboardOrMouse || !hasGamepad)
            {
                throw new InvalidOperationException(
                    $"Player/{actionName} 必须同时提供 keyboard/mouse 与 gamepad binding。");
            }
        }
        ...
        /// <summary>
        /// 记录会明确切换Player输入设备的按下事件，不在Input System callback内重解析bindings。
        /// </summary>
        /// <remarks>
        /// Keyboard/Mouse作为一个设备集合；Gamepad按exact instance配对。未配对的虚拟Gamepad、
        /// Joystick、XR与Touch即使持续上报轴值，也不能参与current Player action解析。
        /// </remarks>
        /// <param name="eventPtr">Input System正在分发的只读设备事件。</param>
        /// <param name="device">事件绑定的exact设备。</param>
        private void OnInputEvent(InputEventPtr eventPtr, InputDevice device)
        {
            if (!_initialized || _stopped || device == null ||
                (eventPtr.type != StateEvent.Type &&
            ...
            {
                if (pressed != null || HasMeaningfulMouseDelta(eventPtr, device))
                {
                    _pendingGameplayGamepad = null;
                    _gameplayDevicePairingPending = true;
                }

                return;
            }

            if (device is Gamepad && pressed != null)
            {
                _pendingGameplayGamepad = device;
                _gameplayDevicePairingPending = true;
            }
        }

        /// <summary>判断一个Mouse事件是否包含真实delta，允许只移动鼠标也切回Keyboard/Mouse。</summary>
        /// <param name="eventPtr">待检查的state或delta event。</param>
        /// <param name="device">事件设备；非Mouse时稳定返回false。</param>
        /// <returns>Mouse delta相对上一设备状态发生非零变化时返回true。</returns>
        private static bool HasMeaningfulMouseDelta(
            ...
            InputDevice device)
        {
            ...
            foreach (var control in eventPtr.EnumerateChangedControls(
                         device,
                         magnitudeThreshold: 0.01f))
            {
                if (control == mouse.delta)
                {
                    return true;
                }
            }
        ...
        /// <summary>在Input System callback结束后的帧末原子提交一次exact设备配对。</summary>
        private void CommitPendingGameplayDevicePairing()
        {
            if (!_gameplayDevicePairingPending || !_initialized || _stopped)
            {
                return;
            }

            var requestedGamepad = _pendingGameplayGamepad;
            _pendingGameplayGamepad = null;
            _gameplayDevicePairingPending = false;
            if (ReferenceEquals(_pairedGameplayGamepad, requestedGamepad))
            {
                return;
            }

            PairPlayerMapToGameplayDevice(requestedGamepad);
            _gameplaySampleWarmupPending = true;
        }

        /// <summary>
        /// 把runtime Player map限制到current Keyboard/Mouse或一个exact Gamepad。
        /// </summary>
        /// <param name="gamepad">目标exact Gamepad；null选择current Keyboard与Mouse。</param>
        private void PairPlayerMapToGameplayDevice(InputDevice gamepad)
        {
            if (_playerActionMap == null)
            {
                throw new InvalidOperationException(
                    "Player action map不存在，不能提交gameplay设备配对。");
            }

            if (gamepad != null && gamepad is not Gamepad)
            {
                throw new ArgumentException(
                    "Gameplay exact device只能是Gamepad。",
                    nameof(gamepad));
            }

            if (gamepad != null)
            {
                _playerActionMap.devices = new[] { gamepad };
                _pairedGameplayGamepad = gamepad;
                return;
            }
            ...
            {
                _playerActionMap.devices = new InputDevice[] { keyboard, mouse };
            }
            else if (keyboard != null)
            {
                _playerActionMap.devices = new InputDevice[] { keyboard };
            }
            else if (mouse != null)
            {
                _playerActionMap.devices = new InputDevice[] { mouse };
            }
            else
            {
                _playerActionMap.devices = Array.Empty<InputDevice>();
            }

            _pairedGameplayGamepad = null;
        }

        /// <summary>销毁 runtime clone 并清空全部派生 action map 引用。</summary>
        private void DestroyRuntimeInputClone()
        {
            if (_inputEventSubscribed)
            {
                InputSystem.onEvent -= OnInputEvent;
                _inputEventSubscribed = false;
            }
