...
namespace IHomeland.Client.Application.Battle
{
    ...
    internal sealed class GameplayPrediction : IAppTickable
    {
        ...
        /// <summary>
        /// 在authority pacing等待期间保留有限clock credit，避免把正常等待误判为render overrun。
        /// </summary>
        private void ClampAuthorityPacedAccumulatorLocked()
        {
            var maximumMilliseconds =
                (double)_policy.MaximumInputStepsPerFrame *
            ...
            ClientBattleSemanticInput input)
        {
            ...
            var commands = new List<ClientBattleInputCommand>(6)
            {
                CreateCommand(
                    inputTick,
                    simulationTick,
        ...
        private void RebuildPredictionLocked()
        {
            ...
            var groupStart = _authoritativeState;
            var groupStartIndex = 0;
            var simulationTick = 0UL;
            for (var index = 0; index < _inputHistory.Count; index++)
            {
                ...
                if (index == 0 || frame.SimulationTick != simulationTick)
                {
                    if (index != 0 && frame.SimulationTick <= simulationTick)
                    {
                        throw new InvalidOperationException(
        ...
        /// <param name="firstFrameIndex">同组首个未确认frame在history中的索引。</param>
        /// <param name="lastFrameIndex">本次fold包含的同组末尾frame索引。</param>
        /// <returns>应用同组last move/aim与OR jump后的单次simulation state。</returns>
            ...
            int lastFrameIndex)
        {
            var moveX = 0;
            var moveY = 0;
            var yaw = current.Transform.YawMillidegrees;
            var jump = false;
            var simulationTick =
                _inputHistory[firstFrameIndex].SimulationTick;
            for (var frameIndex = firstFrameIndex;
                 frameIndex <= lastFrameIndex;
                 frameIndex++)
            {
                var frame = _inputHistory[frameIndex];
                if (frame.SimulationTick != simulationTick)
                {
                    throw new InvalidOperationException(
                        "Prediction fold crossed a SimulationTick boundary.");
                }
            ...
            return new ClientPredictedActorState(
                _inputHistory[lastFrameIndex].InputTick,
                next,
                grounded);
        }
