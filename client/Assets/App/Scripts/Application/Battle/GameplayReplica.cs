...
namespace IHomeland.Client.Application.Battle
{
    ...
    internal sealed class GameplayReplica
    {
        ...
        /// <summary>保存最后可靠 lifecycle event identity。</summary>
        private ulong _lastLifecycleEventID;

        /// <summary>保存最后可靠 ability event identity。</summary>
        private ulong _lastAbilityEventID;
        ...
        internal void AcceptLifecycle(ClientBattleEntityLifecycle lifecycle)
        {
            ...
            lock (_sync)
            {
                if (lifecycle.BattleGeneration != _battleGeneration ||
                    _battleGeneration == 0 ||
                    lifecycle.EventID <= _lastLifecycleEventID)
                {
                    return;
                }

                if (_lastLifecycleEventID != 0 &&
                    lifecycle.EventID != _lastLifecycleEventID + 1)
                {
                    RequestResyncLocked(
                ...
                if (lifecycle.Kind == ClientBattleEntityLifecycleKind.Spawn)
                {
                    ...
                    if (lifecycle.EntityGeneration != expected ||
                        _entities.ContainsKey(lifecycle.EntityID) ||
                    ...
                    _entities.Add(lifecycle.EntityID, lifecycle.InitialState);
                }
                else
                {
                ...
                _lastLifecycleEventID = lifecycle.EventID;
        ...
        internal void AcceptAbilityEvent(ClientBattleAbilityEvent abilityEvent)
        {
            ...
            lock (_sync)
            {
                if (abilityEvent.BattleGeneration != _battleGeneration ||
                    _battleGeneration == 0 ||
                    abilityEvent.EventID <= _lastAbilityEventID)
                {
                    return;
                }

                if (_lastAbilityEventID != 0 &&
                    abilityEvent.EventID != _lastAbilityEventID + 1)
                {
                    RequestResyncLocked(
                ...
                _lastAbilityEventID = abilityEvent.EventID;
            ...
            IReadOnlyList<ClientBattleSnapshotPartition> partitions)
        {
            ...
            if (!replacement.ContainsKey(_localEntityID))
            {
                throw new ClientBattleReplicaProtocolException(
                    "Full snapshot does not contain actor_slot + 1 local entity.");
            }
            ...
            foreach (var pair in replacement)
            {
                ...
                _lifecycleGenerations.Add(pair.Key, pair.Value.Generation);
            }
        ...
        private void ResetLocked()
        {
            ...
            _lastLifecycleEventID = 0;
            _lastAbilityEventID = 0;
