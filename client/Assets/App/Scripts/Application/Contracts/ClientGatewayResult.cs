...
{
    /// <summary>
    /// 标识外部 gateway 可以安全交给 Application 的失败类别。
    /// </summary>
    internal enum ClientGatewayFailureKind
    {
        /// <summary>
        /// 调用方主动取消等待。
        /// </summary>
    ...
    /// <summary>
    /// 保存不会泄漏底层 exception、body、frame 或 credential 的本地 gateway 失败。
    /// </summary>
    internal sealed class ClientGatewayFailure
    {
        /// <summary>
        ...
        internal ClientGatewayFailure(
            ClientGatewayFailureKind kind,
            string operationID)
        {
            Kind = kind;
        ...
        /// 获取稳定失败类别。
        /// </summary>
        internal ClientGatewayFailureKind Kind { get; }
    ...
    /// <summary>
    /// 表示 gateway 调用的成功值、服务端错误或本地失败三选一结果。
    /// </summary>
    /// <typeparam name="T">成功时返回的不可变投影类型。</typeparam>
    internal sealed class ClientGatewayResult<T>
    {
        /// <summary>
        ...
        private ClientGatewayResult(T value, ClientServerError serverError, ClientGatewayFailure failure)
        {
            Value = value;
        ...
        internal static ClientGatewayResult<T> Success(T value)
        {
            ...
            return new ClientGatewayResult<T>(value, null, null);
        }
        ...
        internal static ClientGatewayResult<T> Rejected(ClientServerError error)
        {
            return new ClientGatewayResult<T>(default, error ?? throw new ArgumentNullException(nameof(error)), null);
        }
        ...
        internal static ClientGatewayResult<T> Failed(ClientGatewayFailure failure)
        {
            return new ClientGatewayResult<T>(default, null, failure ?? throw new ArgumentNullException(nameof(failure)));
        }
    }
    ...
    /// </summary>
    internal readonly struct ClientGatewayEmpty
    {
    }
}
