...
# Invoke-Generation 统一重建服务端 Go 与客户端 Unity C# 协议产物。
...
function Invoke-ProtocolCommand {
    ...
    switch ($Command) {
        ...
        "generate" {
            ...
            Invoke-GoGeneration $buf
            Write-Step 2 2 "Generating client C# protocol assembly..."
            Invoke-CSharpGeneration $buf
            Write-Host ""
        ...
        "verify" {
            ...
            Write-Step 3 9 "Rebuilding Go and Unity C# protocol code..."
            ...
            Write-Step 7 9 "Checking deterministic Go and C# protocol generation..."
            ...
            Write-LogLabel "OK" "$($goAfter.Count) Go and $($csharpAfter.Count) C# protocol artifacts are deterministic." Green
