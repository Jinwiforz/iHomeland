...
func encodeBinding(binding domain.Binding) []string {
	...
	return []string{
		...
		binding.Role().String(), binding.WorldID().String(), visitID, binding.Purpose().String(),
...
// decodeBindingReply 从 code 后的 16 个 binding 字段恢复 domain 值。
func decodeBindingReply(items []string) (domain.Binding, error) {
	...
	instanceID, err := placement.NewWorldInstanceID(items[7])
	if err != nil {
		return domain.Binding{}, err
	}
	nodeID, err := placement.NewRuntimeNodeID(items[8])
	if err != nil {
		return domain.Binding{}, err
	}
	generationValue, err := strconv.ParseUint(items[9], 10, 64)
	if err != nil {
		return domain.Binding{}, err
	}
	...
	fenceValue, err := strconv.ParseUint(items[10], 10, 64)
	if err != nil {
		return domain.Binding{}, err
	}
	...
	port, err := strconv.ParseUint(items[13], 10, 16)
	if err != nil {
		return domain.Binding{}, err
	}
	endpoint, err := session.NewEndpoint(session.ChannelTLSTCP, items[12], uint16(port))
	if err != nil {
		return domain.Binding{}, err
	}
	issuedUS, err := strconv.ParseInt(items[14], 10, 64)
	if err != nil {
		return domain.Binding{}, err
	}
	expiresUS, err := strconv.ParseInt(items[15], 10, 64)
	if err != nil {
		return domain.Binding{}, err
	}
	return domain.NewBinding(playerID, sessionID, session.Epoch(epochValue), role, worldID, visitID, purpose, stamp, endpoint, time.UnixMicro(issuedUS), time.UnixMicro(expiresUS))
}
